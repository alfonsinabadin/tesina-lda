...
La técnica de _latent Dirichlet allocation_ (LDA) fue introducida por David M. Blei, Andrew Y. Ng y Michael I. Jordan (2003) como un modelo generativo probabilístico para colecciones de datos discretos, como lo son los corpus textuales. El objetivo central de LDA es descubrir temas latentes o escondidos dentro de un conjunto de documentos bajo la hipótesis de que cada documento está compuesto por una mezcla de varios temas, y que cada tema está definido por una distribución de palabras.

En LDA, una palabra se denomina como la unidad básica de los datos, es un elemento de un vocabulario indexado por $\{1,2,...,V\}$ y se representa como un vector de base unitaria de dimensión $V$. En este sentido, la palabra en la posición $v$ del vocabulario, es un vector $w$ donde $w_v=1$ y $w_u = 0$ $\forall u \neq v$. Teniendo esta definición, un documento es una secuencia de $N$ palabras $\boldsymbol w =(w_1,w_2,...,w_N)$ y un corpus es una colección de $M$ documentos $D=\{\boldsymbol w _1, \boldsymbol w _2,..., \boldsymbol w _M\}$

El modelo define a un tema como una distribución de probabilidad sobre un vocabulario, es decir, una lista de palabras posibles donde cada una tiene una probabilidad asociada. Así, si se presenta un tema que tiene probabilidad alta en palabras como "investigación", "datos", "experimento" o "célula" y una baja probabilidad en palabras como "fútbol" o "presidente", se puede definir como _ciencia_. LDA solo ve patrones de palabras que coexisten y luego, el ojo humano, le pone la etiqueta. 

LDA asume el siguiente proceso generativo para cada documento $\boldsymbol w$ en $D$:

\begin{enumerate}
\item Se selecciona una cantidad de palabras $N\sim Poisson(\xi)$,
\item se selecciona una distribución de temas $\theta\sim Dirichlet(\boldsymbol\alpha)$,
\item para cada posición $n$ en el documento:
\begin{enumerate}
\item se selecciona un tema $z_n \sim Multinomial(\boldsymbol\theta)$,
\item se selecciona una palabra $w_n$ de $p(w_n|z_n, \boldsymbol\beta)$, una función de probabilidad condicionada por el tema.
\end{enumerate}
\end{enumerate}

Asi, se define la distribución conjunta de la mezcla de temas, el conjunto de temas y el conjunto de palabras como

$$p(\boldsymbol\theta, \boldsymbol z, \boldsymbol w | \boldsymbol\alpha, \boldsymbol\beta )=p(\boldsymbol\theta|\boldsymbol\alpha)\times p(\boldsymbol z|\boldsymbol\theta) \times p(\boldsymbol w|\boldsymbol\beta, \boldsymbol z)$$
...
Sea $\boldsymbol\theta$ un vector aleatorio de dimensión $K>2$ donde $\theta_i\in[0,1]$ $\forall i=1, 2,...,K$ y $\sum_{i=1}^K \theta_i = 1$, se puede decir que $\boldsymbol\theta\sim Dirichlet(\boldsymbol\alpha)$ donde $\boldsymbol\alpha$ es un vector de probabilidades de dimensión $K$.
...
Esta distribución es una generalización de la distribución Beta, y es llamada la distribución de distribuciones.
